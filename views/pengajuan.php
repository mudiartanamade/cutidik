...
// --- BAGIAN 2: PERSIAPAN DATA VIEW & FILTER ---
...
// --- MODIFIKASI FILTER & PAGINASI ---
$batas   = 10;
$halaman = isset($_GET['hal']) ? (int)$_GET['hal'] : 1;
$halaman_awal = ($halaman > 1) ? ($halaman * $batas) - $batas : 0;

// Tangkap Input Filter Baru
...
$sql_count = "SELECT COUNT(*) FROM pengajuan_cuti pc JOIN pegawai p ON pc.pegawai_id = p.id" . $where_clause;
...
// Ambil Data Riwayat
$sql_data = "SELECT pc.*, p.nama, p.nip FROM pengajuan_cuti pc JOIN pegawai p ON pc.pegawai_id = p.id" . $where_clause . " ORDER BY pc.id DESC LIMIT $halaman_awal, $batas";
$stmt_data = $conn->prepare($sql_data);
$stmt_data->execute($params);
$riwayat = $stmt_data->fetchAll();

// String URL untuk navigasi paginasi tetap membawa filter
$filter_url = "&cari=$cari&f_jenis=$f_jenis&f_tgl_dari=$f_tgl_dari&f_tgl_sampai=$f_tgl_sampai";
?>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            ...
            <div class="p-6 border-b border-slate-100 bg-slate-50/50">
                <form action="" method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4">
                    ...
                    <div>
                        <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Cari Guru/KS</label>
                        <div class="relative">
                            <span class="absolute left-3 top-2.5 text-slate-400"><i class="fa-solid fa-search"></i></span>
                            <input type="text" name="cari" value="<?= $cari ?>" placeholder="Nama / NIP..." class="w-full pl-9 pr-4 py-2 border border-slate-300 rounded-lg text-sm focus:border-blue-500 transition outline-none">
                        </div>
                    </div>

                    <div>
                        <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Jenis Cuti</label>
                        <select name="f_jenis" class="w-full border border-slate-300 rounded-lg py-2 px-3 text-sm focus:border-blue-500 outline-none">
                            <option value="">Semua Jenis</option>
                            <?php foreach($jenis_opsi as $j): ?>
                                <option value="<?= $j['nama_cuti'] ?>" <?= ($f_jenis == $j['nama_cuti']) ? 'selected' : '' ?>><?= $j['nama_cuti'] ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>

                    <div class="md:col-span-2">
                        <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Rentang Tanggal Mulai</label>
                        <div class="flex items-center gap-2">
                            <input type="date" name="f_tgl_dari" value="<?= $f_tgl_dari ?>" class="w-full border border-slate-300 rounded-lg py-1.5 px-3 text-sm focus:border-blue-500 outline-none">
                            <span class="text-slate-400 text-xs font-bold">s/d</span>
                            <input type="date" name="f_tgl_sampai" value="<?= $f_tgl_sampai ?>" class="w-full border border-slate-300 rounded-lg py-1.5 px-3 text-sm focus:border-blue-500 outline-none">
                        </div>
                    </div>

                    <div class="flex items-end gap-2">
                        <button type="submit" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-lg text-sm font-bold transition shadow-sm">
                            <i class="fa-solid fa-filter mr-1"></i> Filter
                        </button>
                        
                        <button type="button" onclick="exportExcel()" class="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-bold transition shadow-sm" title="Unduh Excel">
                            <i class="fa-solid fa-file-excel"></i>
                        </button>

                        <?php if(!empty($cari) || !empty($f_jenis) || !empty($f_tgl_dari)): ?>
                            <a href="?page=pengajuan&tab=riwayat" class="px-3 py-2 bg-slate-200 hover:bg-slate-300 text-slate-600 rounded-lg text-sm transition" title="Reset Filter">
                ...
                <table class="w-full text-sm text-left text-slate-600">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <?php if(count($riwayat) == 0): ?>
                            <tr><td colspan="5" class="p-8 text-center text-slate-400 italic">Data pengajuan tidak ditemukan.</td></tr>
                        <?php endif; ?>
            ...
            <div class="p-4 border-t border-slate-100 bg-slate-50 flex justify-center gap-2">
                <?php if($halaman > 1): ?>
                    <a href="?page=pengajuan&tab=riwayat<?= $filter_url ?>&hal=<?= $halaman - 1 ?>" class="px-3 py-1 bg-white border border-slate-300 rounded text-sm hover:bg-slate-100 text-slate-600">Prev</a>
                <?php endif; ?>
                ...
                <?php for($x = 1; $x <= $total_halaman; $x++): ?>
                    ...
                    <?php else: ?>
                        <a href="?page=pengajuan&tab=riwayat<?= $filter_url ?>&hal=<?= $x ?>" class="px-3 py-1 bg-white border border-slate-300 rounded text-sm hover:bg-slate-100 text-slate-600"><?= $x ?></a>
                    <?php endif; ?>
                <?php endfor; ?>

                <?php if($halaman < $total_halaman): ?>
                    <a href="?page=pengajuan&tab=riwayat<?= $filter_url ?>&hal=<?= $halaman + 1 ?>" class="px-3 py-1 bg-white border border-slate-300 rounded text-sm hover:bg-slate-100 text-slate-600">Next</a>
                <?php endif; ?>
            </div>
            <?php endif; ?>
        </div>
...
<script>
    ...
    function exportExcel() {
        const cari = document.querySelector('input[name="cari"]').value;
        const jenis = document.querySelector('select[name="f_jenis"]').value;
        const dari = document.querySelector('input[name="f_tgl_dari"]').value;
        const sampai = document.querySelector('input[name="f_tgl_sampai"]').value;

        // Redirect ke file PHP export (pastikan file export_pengajuan.php sudah dibuat)
        window.location.href = `/views/export_pengajuan.php?cari=${cari}&f_jenis=${jenis}&f_tgl_dari=${dari}&f_tgl_sampai=${sampai}`;
    }
